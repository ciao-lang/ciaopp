...
:- module(domains,[
	...
	amgu/5,
	call_to_entry/10, % TODO:[new-resources] (extra)
	call_to_entry/9,
	exit_to_prime/8,
	project/6, % TODO:[new-resources] (extra)
	project/5,
	extend/6, % TODO:[new-resources] (extra)
	extend/5,
	widen/4,
	dual_widen/4,
	widencall/4,
...
%	lub_all/4,
	...
	call_to_success_fact/10, % TODO:[new-resources] (extra)
	body_succ_builtin/9,
	special_builtin/6,
	...
	info_to_asub/5,
	info_to_asub/6, % TODO:[new-resources] (extra)
	info_to_asub/7, % TODO:[new-resources] (extra)
	full_info_to_asub/5, % TODO:[new-resources] (extra arg)
	asub_to_info/5,
	asub_to_out/5,
	asub_to_native/5,
	unknown_call/5,
	...
	unknown_entry/3,
	empty_entry/3,
	% too particular operations: % TODO: [IG] move?
	collect_types_in_abs/4,  % TODO: [IG] only used in typeslib/dumper.pl
	rename_types_in_abs/4,  % TODO: [IG] only used in typeslib/dumper.pl
...
%	del_impose_cond/5,
	...
	[assertions,regtypes,isomodes,nativeprops, ciaopp(ciaopp_options)
	]).
...
% TODO: [IG] move to each of the implementations
aidomain(pd).
aidomain(pdb).
% gr and sharing
aidomain(def).
...
:- endif.
amgu(AbsInt,Sg,Head,ASub,NewASub):-
...
call_to_entry(etermsvar,_Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo):- !,
	etermsvar_call_to_entry(Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo).
...
:- if(defined(has_ciaopp_extra)).
call_to_entry(res_plai,Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo):- !,
	res_plai_call_to_entry(Sv,Sg,Hv,Head,no_provided,Fv,Proj,Entry,ExtraInfo).
call_to_entry(res_plai_stprf,Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo):- !,
	res_plai_stprf_call_to_entry(Sv,Sg,Hv,Head,no_provided,Fv,Proj,Entry,ExtraInfo).
call_to_entry(sized_types,Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo):- !,
        sized_types_call_to_entry(Sv,Sg,Hv,Head,no_provided,Fv,Proj,Entry,ExtraInfo).
:- endif.
call_to_entry(AbsInt,Sv,Sg,Hv,Head,Fv,Proj,Entry,ExtraInfo) :-
        is_nonrel_domain(AbsInt), !,
...
exit_to_prime(etermsvar,Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime):- !,
	etermsvar_exit_to_prime(Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
exit_to_prime(res_plai,Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime):- !,
	res_plai_exit_to_prime(Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime).
exit_to_prime(res_plai_stprf,Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime):- !,
	res_plai_stprf_exit_to_prime(Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime).
exit_to_prime(sized_types,Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime):- !,
        sized_types_exit_to_prime(Sg,Hv,Head,Sv,Exit,ExtraInfo,Prime).
:- endif.
...
project(gr,_,Vars,_,ASub,Proj) :- !,
	gr_project(ASub,Vars,Proj).
:- if(defined(has_ciaopp_extra)).
project(java_nullity,_,Vars,_,ASub,Proj) :- !,
	java_nullity_project(ASub,Vars,Proj).
:- endif.
project(share,_,Vars,_,ASub,Proj) :- !,
	share_project(Vars,ASub,Proj).
:- if(defined(has_ciaopp_extra)).
project(bshare,_,Vars,_,ASub,Proj) :- !,
	bshare_project(Vars,ASub,Proj).
:- endif.
project(share_amgu,_,Vars,_,ASub,Proj) :- !,
	share_project(Vars,ASub,Proj).
project(share_clique,_,Vars,_,ASub,Proj) :- !,
	share_clique_project(Vars,ASub,Proj).
project(share_clique_1,_,Vars,_,ASub,Proj) :- !,
	share_clique_1_project(Vars,ASub,Proj).
project(share_clique_def,_,Vars,_,ASub,Proj) :- !,
	share_clique_def_project(Vars,ASub,Proj).
project(sharefree_clique_def,_,Vars,_,ASub,Proj) :- !,
	sharefree_clique_def_project(Vars,ASub,Proj).
project(sharefree_amgu,_,Vars,_,ASub,Proj) :- !,
	shfr_project(ASub,Vars,Proj).
project(shfrlin_amgu,_,Vars,_,ASub,Proj) :- !,
	shfrlin_project(ASub,Vars,Proj).
project(sharefree_clique,_,Vars,_,ASub,Proj) :- !,
	sharefree_clique_project(ASub,Vars,Proj).
project(shfr,_,Vars,_,ASub,Proj) :- !,
	shfr_project(ASub,Vars,Proj).
project(shfret,_,Vars,_,ASub,Proj) :- !,
	shfret_project(ASub,Vars,Proj).
project(shfrnv,_,Vars,_,ASub,Proj) :- !,
	shfr_project(ASub,Vars,Proj).
project(son,_,Vars,_,ASub,Proj) :- !,
	son_project(Vars,ASub,Proj).
:- if(defined(has_ciaopp_extra)).
project(oo_son,_,Vars,_,ASub,Proj) :- !,
	oo_son_project(ASub,Vars,Proj).
project(oo_shnltau,_,Vars,_,ASub,Proj) :- !,
	oo_shnltau_project(ASub,Vars,Proj).
:- endif.
project(shareson,_,Vars,_,ASub,Proj) :- !,
	shareson_project(Vars,ASub,Proj).
project(shfrson,_,Vars,_,ASub,Proj) :- !,
	shfrson_project(Vars,ASub,Proj).
project(path,_,Vars,_,ASub,Proj) :- !,
	path_project(Vars,ASub,Proj).
project(depth,_,Vars,_,ASub,Proj) :- !,
	depthk_project(Vars,ASub,Proj).
project(depth,_,Vars,_,ASub,Proj) :- !,
	depthk_project(Vars,ASub,Proj).
project(eterms,_,Vars,_,ASub,Proj) :- !,
	eterms_project(Vars,ASub,Proj).
project(etermsvar,_,Vars,_,ASub,Proj) :- !,
	etermsvar_project(Vars,ASub,Proj).
project(svterms,_,Vars,_,ASub,Proj) :- !,
	svterms_project(Vars,ASub,Proj).
project(terms,_,Vars,_,ASub,Proj) :- !,
	terms_project(Vars,ASub,Proj).
project(deftypes,_,Vars,_,ASub,Proj) :- !,
	deftypes_project(Vars,ASub,Proj).
:- if(defined(has_ciaopp_extra)).
project(oo_types,_,Vars,_,ASub,Proj) :- !,
	oo_types_project(ASub,Vars,Proj).
project(java_cha,_,Vars,_,ASub,Proj) :- !,
	java_cha_project(ASub,Vars,Proj).
:- endif.
:- if(defined(has_ciaopp_extra)).
project(polyhedra,_,Vars,_,ASub,Proj) :- !,
	polyhedra_project(ASub,Vars,Proj).
:- endif.
project(fr,_,Vars,_,ASub,Proj) :- !,
	fr_project(ASub,Vars,Proj).
project(def,_,Vars,_,ASub,Proj) :- !,
	def_project(ASub,Vars,Proj).
project(frdef,_,Vars,_,ASub,Proj) :- !,
	fd_project(ASub,Vars,Proj).
project(aeq,_,Vars,_,ASub,Proj) :- !,
	aeq_project(ASub,Vars,Proj).
project(lsign,_,Vars,HvFv,ASub,Proj) :- !,
	lsign_project(ASub,Vars,HvFv,Proj).
% check that HvFv is sorted!
project(lsigndef,_,Vars,HvFv,ASub,Proj) :- !,
	lsigndef_project(ASub,Vars,HvFv,Proj).
project(lsignshfr,_,Vars,HvFv,ASub,Proj) :- !,
	lsignshfr_project(ASub,Vars,HvFv,Proj).
project(difflsign,_,Vars,HvFv,ASub,Proj) :- !,
	simple_lsign_project(ASub,Vars,HvFv,Proj).
project(sha,_,Vars,_HvFv,ASub,Proj) :- !,
	sha_project(ASub,Vars,Proj).
:- if(defined(has_ciaopp_extra)).
project(nf,_,Vars,_HvFv,ASub,Proj) :- !,
	nf_project(ASub,Vars,Proj).
project(det,_,Vars,_HvFv,ASub,Proj) :- !,
	det_project(ASub,Vars,Proj).
:- endif.
:- if(defined(has_ciaopp_extra)).
project(res_plai,Sg,Vars,_HvFv,ASub,Proj) :- !,
	res_plai_project(ASub,Sg,Vars,Proj).
project(res_plai_stprf,Sg,Vars,_HvFv,ASub,Proj) :- !,
	res_plai_stprf_project(ASub,Sg,Vars,Proj).
project(sized_types,Sg,Vars,_HvFv,ASub,Proj) :- !,
	sized_types_project(ASub,Sg,Vars,Proj).
:- endif.
...
widencall(etermsvar,Prime0,Prime1,NewPrime):- !,
	etermsvar_widencall(Prime0,Prime1,NewPrime).
...
%:- if(defined(has_ciaopp_extra)).
%widencall(res_plai,Prime0,Prime1,NewPrime):- !,
%	res_plai_widencall(Prime0,Prime1,NewPrime).
...
widen(etermsvar,Prime0,Prime1,NewPrime):- !,
...
:- if(defined(has_ciaopp_extra)).
widen(res_plai,Prime0,Prime1,NewPrime):- !,
	res_plai_widen(Prime0,Prime1,NewPrime).
widen(res_plai_stprf,Prime0,Prime1,NewPrime):- !,
	res_plai_stprf_widen(Prime0,Prime1,NewPrime).
widen(sized_types,Prime0,Prime1,NewPrime):- !,
        sized_types_widen(Prime0,Prime1,NewPrime).
:- endif.
...
compute_lub(etermsvar,ListASub,LubASub) :- !,
	etermsvar_compute_lub(ListASub,LubASub).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
compute_lub(res_plai,ListASub,GlbASub) :- !,
	res_plai_compute_lub(ListASub,GlbASub).
compute_lub(res_plai_stprf,ListASub,GlbASub) :- !,
	res_plai_stprf_compute_lub(ListASub,GlbASub).
compute_lub(sized_types,ListASub,GlbASub) :- !,
	sized_types_compute_lub(ListASub,GlbASub).
:- endif.
...
:- endif.
:- if(defined(has_ciaopp_extra)).
compute_clauses_lub(res_plai,Proj,ASub,Lub):- !,
	res_plai_compute_clauses_lub(ASub,Proj,Lub).
compute_clauses_lub(res_plai_stprf,Proj,ASub,Lub):- !,
	res_plai_stprf_compute_clauses_lub(ASub,Proj,Lub).
compute_clauses_lub(sized_types,Proj,ASub,Lub):- !,
	sized_types_compute_clauses_lub(ASub,Proj,Lub).
:- endif.
...
%:- if(defined(has_ciaopp_extra)).
%compute_clauses_glb(res_plai,Proj,ASub,Lub):- !,
%	res_plai_compute_clauses_lub(ASub,Proj,Lub).
%:- endif.
...
identical_abstract(etermsvar,ASub1,ASub2) :- !,
...
:- endif.
:- if(defined(has_ciaopp_extra)).
identical_abstract(res_plai,ASub1,ASub2) :- !,
	res_plai_identical_abstract(ASub1,ASub2).
identical_abstract(res_plai_stprf,ASub1,ASub2) :- !,
	res_plai_stprf_identical_abstract(ASub1,ASub2).
identical_abstract(sized_types,ASub1,ASub2) :- !,
	sized_types_identical_abstract(ASub1,ASub2).
:- endif.
...
abs_sort(etermsvar,ASub,ASub_s) :- !,
	etermsvar_sort(ASub,ASub_s).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
abs_sort(res_plai,ASub,ASub_s) :- !,
	res_plai_sort(ASub,ASub_s).
abs_sort(res_plai_stprf,ASub,ASub_s) :- !,
	res_plai_stprf_sort(ASub,ASub_s).
abs_sort(sized_types,ASub,ASub_s) :- !,
	sized_types_sort(ASub,ASub_s).
:- endif.
...
extend(pdb,_,Prime,Sv,Call,Succ) :- !,
	pdb_extend(Prime,Sv,Call,Succ).
extend(gr,_,Prime,Sv,Call,Succ) :- !,
	gr_extend(Prime,Sv,Call,Succ).
:- if(defined(has_ciaopp_extra)).
extend(java_nullity,_,Prime,Sv,Call,Succ) :- !,
	java_nullity_extend(Prime,Sv,Call,Succ).
:- endif.
extend(share,_,Prime,Sv,Call,Succ) :- !,
	share_extend(Prime,Sv,Call,Succ).
extend(share_amgu,_,Prime,Sv,Call,Succ) :- !,
	share_extend(Prime,Sv,Call,Succ).
extend(share_clique,_,Prime,Sv,Call,Succ) :- !,
	share_clique_extend(Prime,Sv,Call,Succ).
extend(share_clique_1,_,Prime,Sv,Call,Succ) :- !,
	share_clique_1_extend(Prime,Sv,Call,Succ).
extend(share_clique_def,_,Prime,Sv,Call,Succ) :- !,
	share_clique_def_extend(Prime,Sv,Call,Succ).
extend(sharefree_clique_def,_,Prime,Sv,Call,Succ) :- !,
	sharefree_clique_def_extend(Prime,Sv,Call,Succ).
extend(sharefree_amgu,_,Prime,Sv,Call,Succ) :- !,
	shfr_extend(Prime,Sv,Call,Succ).
extend(shfrlin_amgu,_,Prime,Sv,Call,Succ) :- !,
	shfrlin_extend(Prime,Sv,Call,Succ).
extend(sharefree_clique,_,Prime,Sv,Call,Succ) :- !,
	sharefree_clique_extend(Prime,Sv,Call,Succ).
extend(shfr,_,Prime,Sv,Call,Succ) :- !,
	shfr_extend(Prime,Sv,Call,Succ).
extend(shfret,_,Prime,Sv,Call,Succ) :- !,
	shfret_extend(Prime,Sv,Call,Succ).
extend(shfrnv,_,Prime,Sv,Call,Succ) :- !,
	shfrnv_extend(Prime,Sv,Call,Succ).
extend(son,_,Prime,Sv,Call,Succ) :- !,
	son_extend(Prime,Sv,Call,Succ).
:- if(defined(has_ciaopp_extra)).
extend(oo_son,_,Prime,Sv,Call,Succ) :- !,
	oo_son_extend(Prime,Sv,Call,Succ).
extend(oo_shnltau,_,Prime,Sv,Call,Succ) :- !,
	oo_shnltau_extend(Prime,Sv,Call,Succ).
:- endif.
extend(shareson,_,Prime,Sv,Call,Succ) :- !,
	shareson_extend(Prime,Sv,Call,Succ).
extend(shfrson,_,Prime,Sv,Call,Succ) :- !,
	shfrson_extend(Prime,Sv,Call,Succ).
extend(path,_,Prime,Sv,Call,Succ) :- !,
	path_extend(Prime,Sv,Call,Succ).
extend(depth,_,Prime,Sv,Call,Succ) :- !,
	depthk_extend(Prime,Sv,Call,Succ).
extend(eterms,_,Prime,Sv,Call,Succ) :- !,
	eterms_extend(Prime,Sv,Call,Succ).
extend(etermsvar,_,Prime,Sv,Call,Succ) :- !,
	etermsvar_extend(Prime,Sv,Call,Succ).
extend(svterms,_,Prime,Sv,Call,Succ) :- !,
	svterms_extend(Prime,Sv,Call,Succ).
extend(terms,_,Prime,Sv,Call,Succ) :- !,
	terms_extend(Prime,Sv,Call,Succ).
extend(ptypes,_,Prime,Sv,Call,Succ) :- !,
	terms_extend(Prime,Sv,Call,Succ).
extend(deftypes,_,Prime,Sv,Call,Succ) :- !,
	deftypes_extend(Prime,Sv,Call,Succ).
:- if(defined(has_ciaopp_extra)).
extend(oo_types,_,Prime,Sv,Call,Succ) :- !,
	oo_types_extend(Prime,Sv,Call,Succ).
extend(java_cha,_,Prime,Sv,Call,Succ) :- !,
	java_cha_extend(Prime,Sv,Call,Succ).
:- endif.
:- if(defined(has_ciaopp_extra)).
extend(polyhedra,_,Prime,Sv,Call,Succ) :- !,
	polyhedra_extend(Prime,Sv,Call,Succ).
:- endif.
extend(fr,_,Prime,Sv,Call,Succ) :- !,
	fr_extend(Prime,Sv,Call,Succ).
extend(def,_,Prime,_Sv,Call,Succ) :- !,
	def_extend(Prime,Call,Succ).
extend(frdef,_,Prime,Sv,Call,Succ) :- !,
	fd_extend(Prime,Sv,Call,Succ).
extend(aeq,_,Prime,_Sv,Call,Succ) :- !,
	aeq_extend(Prime,Call,Succ).
extend(lsign,_,Prime,_Sv,Call,Succ) :- !,
	lsign_extend(Prime,Call,Succ).
extend(lsigndef,_,Prime,Sv,Call,Succ) :- !,
	lsigndef_extend(Prime,Sv,Call,Succ).
extend(lsignshfr,_,Prime,Sv,Call,Succ) :- !,
	lsignshfr_extend(Prime,Sv,Call,Succ).
extend(difflsign,_,Prime,_Sv,Call,Succ) :- !,
	simple_lsign_extend(Prime,Call,Succ).
extend(sha,_,Prime,Sv,Call,Succ) :- !,
	sha_extend(Prime,Sv,Call,_Proj,Succ).
:- if(defined(has_ciaopp_extra)).
extend(nf,_,Prime,Sv,Call,Succ) :- !,
	nf_extend(Prime,Sv,Call,Succ).
extend(det,_,Prime,Sv,Call,Succ) :- !,
	det_extend(Prime,Sv,Call,Succ).
:- endif.
:- if(defined(has_ciaopp_extra)).
extend(res_plai,Sg,Prime,Sv,Call,Succ) :- !,
	res_plai_extend(Sg,Prime,Sv,Call,Succ).
extend(res_plai_stprf,Sg,Prime,Sv,Call,Succ) :- !,
	res_plai_stprf_extend(Sg,Prime,Sv,Call,Succ).
extend(sized_types,Sg,Prime,Sv,Call,Succ) :- !,
        sized_types_extend(Sg,Prime,Sv,Call,Succ).
:- endif.
...
less_or_equal(etermsvar,ASub0,ASub1) :- !,
	etermsvar_less_or_equal(ASub0,ASub1).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
less_or_equal(res_plai,ASub0,ASub1) :- !,
	res_plai_less_or_equal(ASub0,ASub1).
less_or_equal(res_plai_stprf,ASub0,ASub1) :- !,
	res_plai_stprf_less_or_equal(ASub0,ASub1).
less_or_equal(sized_types,ASub0,ASub1) :- !,
        sized_types_less_or_equal(ASub0,ASub1).
:- endif.
...
glb(etermsvar,ASub0,ASub1,ASub) :- !,
	etermsvar_glb(ASub0,ASub1,ASub).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
glb(res_plai,ASub0,ASub1,ASub) :- !,
	res_plai_glb(ASub0,ASub1,ASub).
glb(res_plai_stprf,ASub0,ASub1,ASub) :- !,
	res_plai_stprf_glb(ASub0,ASub1,ASub).
glb(sized_types,ASub0,ASub1,ASub) :- !,
        sized_types_glb(ASub0,ASub1,ASub).
:- endif.
...
eliminate_equivalent(eterms,TmpLSucc,LSucc) :- !,
	sort(TmpLSucc,LSucc).
eliminate_equivalent(etermsvar,TmpLSucc,LSucc) :- !,
	sort(TmpLSucc,LSucc).
...
call_to_success_fact(etermsvar,Sg,Hv,Head,Sv,Call,Proj,Prime,Succ) :- !,
...
% TODO:[new-resources] 'no_provided' is strange
:- if(defined(has_ciaopp_extra)).
call_to_success_fact(res_plai,Sg,Hv,Head,Sv,Call,Proj,Prime,Succ) :- !,
	res_plai_call_to_success_fact(Sg,Hv,Head,no_provided,Sv,Call,Proj,Prime,Succ).
call_to_success_fact(res_plai_stprf,Sg,Hv,Head,Sv,Call,Proj,Prime,Succ) :- !,
	res_plai_stprf_call_to_success_fact(Sg,Hv,Head,no_provided,Sv,Call,Proj,Prime,Succ).
call_to_success_fact(sized_types,Sg,Hv,Head,Sv,Call,Proj,Prime,Succ) :- !,
	sized_types_call_to_success_fact(Sg,Hv,Head,no_provided,Sv,Call,Proj,Prime,Succ).
:- endif.
call_to_success_fact(AbsInt,Sg,Hv,Head,Sv,Call,Proj,Prime,Succ):-
        is_nonrel_domain(AbsInt), !,
...
special_builtin(etermsvar,SgKey,Sg,Subgoal,Type,Condvars) :- !,
...
:- endif.
:- if(defined(has_ciaopp_extra)).
special_builtin(res_plai,SgKey,Sg,_Subgoal,Type,Condvars) :- !,
	res_plai_special_builtin(SgKey,Sg,Type,Condvars).
special_builtin(res_plai_stprf,SgKey,Sg,_Subgoal,Type,Condvars) :- !,
	res_plai_stprf_special_builtin(SgKey,Sg,Type,Condvars).
special_builtin(sized_types,SgKey,Sg,_Subgoal,Type,Condvars) :- !,
	sized_types_special_builtin(SgKey,Sg,Type,Condvars).
:- endif.
...
:- if(defined(has_ciaopp_extra)).
split_combined_domain(res_plai,ASub,ASubs,Doms):-
	res_plai_split_combined_domain(ASub,ASubs,Doms).
split_combined_domain(res_plai_stprf,ASub,ASubs,Doms):-
	res_plai_stprf_split_combined_domain(ASub,ASubs,Doms).
split_combined_domain(sized_types,ASub,ASubs,Doms):-
	sized_types_split_combined_domain(ASub,ASubs,Doms).
:- endif.
...
success_builtin(etermsvar,Type,Sv_uns,Condvars,_,Call,Succ) :- !,
...
:- endif.
%:- if(defined(has_ciaopp_extra)).
%success_builtin(res_plai,Type,Sv_uns,Condvars,_,Call,Succ) :- !,
%	res_plai_success_builtin(Type,Sv_uns,Condvars,Call,Succ).
%:- endif.
...
call_to_success_builtin(etermsvar,SgKey,Sg,Sv,Call,Proj,Succ):- !,
	etermsvar_call_to_success_builtin(SgKey,Sg,Sv,Call,Proj,Succ).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
call_to_success_builtin(res_plai,SgKey,Sg,Sv,Call,Proj,Succ):- !,
	res_plai_call_to_success_builtin(SgKey,Sg,Sv,Call,Proj,Succ).
call_to_success_builtin(res_plai_stprf,SgKey,Sg,Sv,Call,Proj,Succ):- !,
	res_plai_stprf_call_to_success_builtin(SgKey,Sg,Sv,Call,Proj,Succ).
call_to_success_builtin(sized_types,SgKey,Sg,Sv,Call,Proj,Succ):- !,
        sized_types_call_to_success_builtin(SgKey,Sg,Sv,Call,Proj,Succ).
:- endif.
call_to_success_builtin(AbsInt,SgKey,Sg,Sv,Call,Proj,Succ):-
        is_nonrel_domain(AbsInt), !,
...
obtain_info(etermsvar,_Prop,Vars,ASub,Info):-
	asub_to_info(etermsvar,ASub,Vars,Info,_CompProps).
:- if(defined(has_ciaopp_extra)).
obtain_info(res_plai,_Prop,Vars,ASub,Info):-
	asub_to_info(res_plai,ASub,Vars,_Info,Info).
obtain_info(res_plai_stprf,_Prop,Vars,ASub,Info):-
	asub_to_info(res_plai_stprf,ASub,Vars,_Info,Info).
obtain_info(sized_types,_Prop,Vars,ASub,Info):-
	asub_to_info(sized_types,ASub,Vars,_Info,Info).
:- endif.
...
input_interface(etermsvar,InputUser,Kind,Struct0,Struct1) :- !,
	etermsvar_input_interface(InputUser,Kind,Struct0,Struct1).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
input_interface(res_plai,InputUser,Kind,Struct0,Struct1) :- !,
	res_plai_input_interface(InputUser,Kind,Struct0,Struct1).
input_interface(res_plai_stprf,InputUser,Kind,Struct0,Struct1) :- !,
	res_plai_stprf_input_interface(InputUser,Kind,Struct0,Struct1).
input_interface(sized_types,InputUser,Kind,Struct0,Struct1) :- !,
	sized_types_input_interface(InputUser,Kind,Struct0,Struct1).
:- endif.
...
input_user_interface(etermsvar,InputUser,Qv,ASub) :- !,
...
asub_to_native(etermsvar,ASub,_Qv,OutputUser,[]) :- !,
	etermsvar_asub_to_native(ASub,no,OutputUser).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
asub_to_native(res_plai,ASub,Qv,OutputUser,Comp) :- !,
	res_plai_asub_to_native(ASub,Qv,no,OutputUser,Comp).
asub_to_native(res_plai_stprf,ASub,Qv,OutputUser,Comp) :- !,
	res_plai_stprf_asub_to_native(ASub,Qv,no,OutputUser,Comp).
asub_to_native(sized_types,ASub,Qv,OutputUser,Comp) :- !,
        sized_types_asub_to_native(ASub,Qv,no,OutputUser,Comp).
:- endif.
...
asub_to_native_out(etermsvar,ASub,_Qv,OutputUser,[]):- !,
	etermsvar_asub_to_native(ASub,yes,OutputUser).
...
:- if(defined(has_ciaopp_extra)).
asub_to_native_out(res_plai,ASub,Qv,OutputUser,Comp):- !,
	res_plai_asub_to_native(ASub,Qv,yes,OutputUser,Comp).
asub_to_native_out(res_plai_stprf,ASub,Qv,OutputUser,Comp):- !,
	res_plai_stprf_asub_to_native(ASub,Qv,yes,OutputUser,Comp).
asub_to_native_out(sized_types,ASub,Qv,OutputUser,Comp):- !,
	sized_types_asub_to_native(ASub,Qv,yes,OutputUser,Comp).
:- endif.
%
asub_to_native_out(AbsInt,ASub,Qv,Info,Comp):-
	asub_to_native(AbsInt,ASub,Qv,Info,Comp).
...
%concrete(etermsvar,Var,ASub,List):-
...
unknown_call(etermsvar,Vars,Call,Succ) :- !,
	etermsvar_unknown_call(Call,Vars,Succ).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
unknown_call(res_plai,Vars,Call,Succ) :- !,
	res_plai_unknown_call(Vars,Call,Succ).
unknown_call(res_plai_stprf,Vars,Call,Succ) :- !,
	res_plai_stprf_unknown_call(Vars,Call,Succ).
unknown_call(sized_types,Vars,Call,Succ) :- !,
        sized_types_unknown_call(Vars,Call,Succ).
:- endif.
...
unknown_entry(etermsvar,Qv,Call) :- !,
	etermsvar_unknown_entry(Qv,Call).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
unknown_entry(res_plai,Qv,Call) :- !,
	res_plai_unknown_entry(Qv,Call).
unknown_entry(res_plai_stprf,Qv,Call) :- !,
	res_plai_stprf_unknown_entry(Qv,Call).
unknown_entry(sized_types,Qv,Call) :- !,
        sized_types_unknown_entry(Qv,Call).
:- endif.
...
empty_entry(etermsvar,Qv,Call) :- !,
	etermsvar_empty_entry(Qv,Call).
...
:- endif.
:- if(defined(has_ciaopp_extra)).
empty_entry(res_plai,Qv,Call) :- !,
	res_plai_empty_entry(Qv,Call).
empty_entry(res_plai_stprf,Qv,Call) :- !,
	res_plai_stprf_empty_entry(Qv,Call).
empty_entry(sized_types,Qv,Call) :- !,
	sized_types_empty_entry(Qv,Call).
:- endif.
...
init_abstract_domain(terms,[widen]):- !,
	push_pp_flag(widen,on).
init_abstract_domain(ptypes,[widen]):- !,
	push_pp_flag(widen,on).
init_abstract_domain(deftypes,[widen]):- !,
	push_pp_flag(widen,on).
:- if(defined(has_ciaopp_extra)).
init_abstract_domain(polyhedra,[widen]):- !, 
        push_pp_flag(widen,on).
:- endif.
init_abstract_domain(eterms,[widen]):- !,
	push_pp_flag(widen,on).
init_abstract_domain(etermsvar,[type_eval,widen]):- !,
	push_pp_flag(type_eval,on),
	push_pp_flag(widen,on).
init_abstract_domain(svterms,[widen]):- !,
	push_pp_flag(widen,on).
:- if(defined(has_ciaopp_extra)).
init_abstract_domain(nf,[variants,widen]):- !,
	push_pp_flag(variants,off),
	push_pp_flag(widen,on).
init_abstract_domain(det,[variants,widen]):- !,
	push_pp_flag(variants,off),
	push_pp_flag(widen,on).
:- endif.
init_abstract_domain(shfret,[variants,widen]):- !,
	push_pp_flag(variants,off),
	push_pp_flag(widen,on).
init_abstract_domain(path,[variants,multi_success]):- !,
	push_pp_flag(variants,off),
	push_pp_flag(multi_success,on).
:- if(defined(has_ciaopp_extra)).
init_abstract_domain(res_plai,[widen]):- !,
	push_pp_flag(widen,on),
%	res_plai:load_resources_modules.
	res_plai:init_abstract_domain.
init_abstract_domain(res_plai_stprf,[widen]):- !,
	push_pp_flag(widen,on),
%	res_plai:load_resources_modules.
	res_plai_stprf:init_abstract_domain.
init_abstract_domain(sized_types,[widen]):- !,
	push_pp_flag(widen,on),
%	sized_types:load_resources_modules.
	sized_types:init_abstract_domain.
:- endif.
init_abstract_domain(lsign,[normalize,variants]):- !,
	push_pp_flag(normalize,on),
	push_pp_flag(variants,off).
% :- if(defined(has_ciaopp_extra)).
% init_abstract_domain(oo_shnltau,[]):- !,
% 	oo_shnltau_init_abstract_domain.
% init_abstract_domain(oo_types,[]):- !,
% 	oo_types_init_abstract_domain.
% :- endif.
init_abstract_domain(nonrel_intervals,[widen]):- !,
        push_pp_flag(widen,on).
...
part_conc(etermsvar,Sg,Subs,NSg,NSubs):-!,
	etermsvar_part_conc(Sg,Subs,NSg,NSubs).
...
multi_part_conc(etermsvar,Sg,Subs,List):-!,
	etermsvar_multi_part_conc(Sg,Subs,List).
...
collect_abstypes_abs(etermsvar,ASub,Types0,Types):-
	etermsvar_collect_abstypes(ASub,Types0,Types).
...
:- if(defined(has_ciaopp_extra)).
collect_abstypes_abs(res_plai,ASub,Types0,Types):-
	res_plai_collect_abstypes(ASub,Types0,Types).
collect_abstypes_abs(res_plai_stprf,ASub,Types0,Types):-
	res_plai_stprf_collect_abstypes(ASub,Types0,Types).
collect_abstypes_abs(sized_types,ASub,Types0,Types):-
	sized_types_collect_abstypes(ASub,Types0,Types).
:- endif.
...
rename_abstypes_abs(etermsvar,ASub,(Types,Names),RenASub):- !,
	etermsvar_rename_abs(ASub,Types,Names,RenASub).
...
rename_abstypes_abs(deftypes,ASub,(Types,Names),RenASub):- !,
	terms_rename_abs(ASub,Types,Names,RenASub).
:- if(defined(has_ciaopp_extra)).
rename_abstypes_abs(res_plai,ASub,(Types,Names),RenASub):- !,
	res_plai_rename_abs(ASub,Types,Names,RenASub).
rename_abstypes_abs(res_plai_stprf,ASub,(Types,Names),RenASub):- !,
	res_plai_stprf_rename_abs(ASub,Types,Names,RenASub).
rename_abstypes_abs(sized_types,ASub,(Types,Names),RenASub):- !,
	sized_types_rename_abs(ASub,Types,Names,RenASub).
:- endif.
...
% and/or these shoudl be moved to the module that implements each domain
