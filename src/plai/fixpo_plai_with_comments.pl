...
:- module(fixpo_plai_with_comments,
    [ query/8,
      init_fixpoint/0,
      cleanup_fixpoint/1,
      entry_to_exit/9
...
% call_to_success(_RFlag,SgKey,Call,Proj,Sg,Sv,AbsInt,_ClId,Succ,List,_F,_N,_Id) :-  
% 	format("Call (\n\t~p\n\t~p\n\t~p\n\t~p\n\t~p\n\t~p\n\t~p\n\t~p\n)",
% 	[SgKey,Call,Proj,Sg,Sv,AbsInt,Succ,List]),
%       fail.

% call_to_success(_RFlag,_SgKey,_Call,_Proj,_Sg,_Sv,_AbsInt,_ClId,_Succ,_List,_F,_N,_Id) :-  
% 	format("-",[]), inc(plai_counter(inc_call,_)),
% 	fail.
...
%w      write('c'),
    ...
    format("call to success tipe _RFlag SgKey",[]),
...
%	format("apply_trusted ~p with ClId = ~p, Proj = ~p\n",[SgKey, ClId, Proj]),
    apply_trusted0(Proj,SgKey,Sg,Sv,AbsInt,ClId,Prime), !,
%	format("Obtain Prime = ~p\n",[Prime]),
...
:- data plai_time/1, plai_counter/2.
:- export(plai_time/1).
:- export(plai_counter/2).
...
init_counters:-
	retractall_fact(plai_counter(_,_)),
	assertz_fact(plai_counter(inc_call,0)),
	assertz_fact(plai_counter(inc_check,0)),
	assertz_fact(plai_counter(inc_ok,0)).

inc(plai_counter(Counter,_)) :-
	plai_counter(Counter,Value),
	retract_fact(plai_counter(Counter,Value)),
	Value1 is Value + 1,
	assertz_fact(plai_counter(Counter,Value1)).