...
%------------------------------------------------------------------%
:- pred eterms_extend(+Sg,+Prime,+Sv,+Call,-Succ): term * absu * list * absu * absu # 
...
:- pred eterms_empty_entry(+Sg,+Vars,-Entry): callable * list * absu # "Gives the
...
eterms_rename_abstypes_abs([],_,[]).
eterms_rename_abstypes_abs([C|Call],Dict,[RenC|RenCall]):-
	Dict = (Types,_Names),
	C = Var:(_Name,Type),
