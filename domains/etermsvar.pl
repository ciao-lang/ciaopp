:- module(etermsvar,[                          
    ...
    etermsvar_collect_abstypes_abs/3,
    etermsvar_rename_abstypes_abs/3,
...
%
%   See `etermsvar.org` file for more details.
%
...
:- pred etermsvar_widen(+Prime0,+Prime1,-NewPrime):
absu * absu * absu #
"
...
etermsvar_call_to_entry(_Sv,Sg,Hv,Head,_K,Fv,Proj,Entry,(yes,Proj)):- 
    ...
    etermsvar_project(Sg,Hv,not_provided_HvFv_u,NewProj,NewProj1),
...
:- pred variables_are_variable_type(+Fv,-ASub) :: list * absu # 
"it assigns the value var_type to the variables in @var{Fv}
...
%------------------------------------------------------------------%
:- pred etermsvar_exit_to_prime(+Sg,+Hv,+Head,+Sv,+Exit,-ExtraInfo,-Prime): list
...
etermsvar_exit_to_prime(Sg,Hv,Head,_Sv,Exit,(yes,Proj),Prime):- !,
    etermsvar_project(Sg,Hv,not_provided_HvFv_u,Exit,BPrime),
...
unify_term_and_type_term_exit(Term1,Tv,Term2,ASub,Proj,NewASub):-
    ...
    apply(ASub0),
    generate_a_type_assigment_special_var(EscTypes,Args,TypeAss),
    ( 
    ...
    ;
        ...
        etermsvar_project(not_provided_Sg,Tv,not_provided_HvFv_u,Subs,NASub),
...
update_names(_,_):-
    error_message("SOMETHING HAS FAILED! See etermsvar domain.", []).
...
%------------------------------------------------------------------%
:- pred etermsvar_project(+Sg,+Vars,+HvFv_u,+Asub,-Proj): term * list * list * absu * absu # 
"@var{Proj} is the result of eliminating from @var{Asub} all
@var{X}:@var{Value} such that @var{X} is not in @var{Vars}".



etermsvar_project(_,_,_,'$bottom',Proj) :- !,
    Proj = '$bottom'.
etermsvar_project(_Sg,Vars,_HvFv_u,ASub,Proj) :- 
    etermsvar_project_aux(Vars,ASub,Proj).

etermsvar_project_aux([],_,Proj):- !,
    Proj = [].
etermsvar_project_aux(_,[],Proj):- !,
    Proj = [].
etermsvar_project_aux([Head1|Tail1],[Head2:Type|Tail2],Proj) :-
    compare(Order,Head1,Head2),
    etermsvar_project_aux_(Order,Head1,Tail1,Head2:Type,Tail2,Proj).

etermsvar_project_aux_(=,_,Tail1,HeadType,Tail2,[HeadType|Proj]) :-
    etermsvar_project_aux(Tail1,Tail2,Proj).
etermsvar_project_aux_(>,Head1,Tail1,_,[Head2:Type|Tail2],Proj) :-
    compare(Order,Head1,Head2),
    etermsvar_project_aux_(Order,Head1,Tail1,Head2:Type,Tail2,Proj).
...
etermsvar_glb(ASub1,ASub2,ASub3):-
    ...
    etermsvar_glbnames(ASub1,ASub33,ASub3).
...
etermsvar_glb0([X:(_N1,T1)|ASub1],[Y:(N2,T2)|ASub2],[X:(N2,T3)|ASub3]):-
    X==Y,
    type_intersection_2_special_var(T1,T2,T3),
    ( 
        T3==bot -> fail 
    ;
...
etermsvar_glb0_without_var([X:(_N1,T1)|ASub1],[Y:(N2,T2)|ASub2],[X:(N2,T2)|ASub3]):-
    X==Y,
    var_type(T1), !,
    etermsvar_glb0_without_var(ASub1,ASub2,ASub3).
etermsvar_glb0_without_var([X:(_N1,T1)|ASub1],[Y:(N2,T2)|ASub2],[X:(N2,T1)|ASub3]):-
    X==Y,
    var_type(T2), !,
    etermsvar_glb0_without_var(ASub1,ASub2,ASub3).
...
etermsvar_glbnames([X:(N1_e,_)|ASub1],[X:(N3_e,T3)|ASub33],[X:(N3,T3)|ASub3]):-
    get_canonical_name(N1_e,N1),
...
%------------------------------------------------------------------%
:- pred etermsvar_unknown_call(+Sg,+Vars,+Call,-Succ): callable * list * absu * absu # 
"Gives the ``top'' value for the variables involved in a 
 literal whose definition is not present, and adds this top value to
 @var{Call}".

etermsvar_unknown_call(_Sg,Vars,Call,Succ):-
...
etermsvar_special_builtin('!/0',_Sg,_,id,[]).
etermsvar_special_builtin('@=</2',_Sg,_,id,[]).
etermsvar_special_builtin('@>/2',_Sg,_,id,[]).
etermsvar_special_builtin('@>=/2',_Sg,_,id,[]).
etermsvar_special_builtin('@</2',_Sg,_,id,[]).
etermsvar_special_builtin('\\==/2',_Sg,_,id,[]).
etermsvar_special_builtin('==/2',_Sg,_,id,[]).
etermsvar_special_builtin('display/1',_Sg,_,id,[]).
etermsvar_special_builtin('var/1',Sg,_,type(T),Condvars):-
    set_var_type(T),
    varset(Sg,Condvars).
etermsvar_special_builtin('nonvar/1',Sg,_,type(T),Condvars):-
    set_ground_type(T),
    varset(Sg,Condvars).
...
etermsvar_special_builtin('number/1',Sg,_,type(T),Condvars):-
    set_numeric_type(T),
    varset(Sg,Condvars).
etermsvar_special_builtin('fail/0',_Sg,_,bot,[]).
etermsvar_special_builtin('true/0',_Sg,_,id,[]).
etermsvar_special_builtin('nl/0',_Sg,_,id,[]).
etermsvar_special_builtin('repeat/0',_Sg,_,id,[]).

etermsvar_special_builtin('erase/1',Sg,_,type(T),Condvars):-
    set_top_type(T),
    varset(Sg,Condvars).

etermsvar_special_builtin(Key,_Sg,Subgoal,special(KKey),[]):-
    etermsvar_very_special_builtin(Key,KKey,Subgoal).

etermsvar_very_special_builtin('=/2','=/2',_).
etermsvar_very_special_builtin('is/2','is/2',_).%:- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('functor/3','functor/3',_):- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('arg/3','arg/3',_):- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('>/2','>/2',_).%:- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('>=/2','>=/2',_).%:- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('=</2','=</2',_).%:- current_pp_flag(type_eval,on).
etermsvar_very_special_builtin('</2','</2',_).%:- current_pp_flag(type_eval,on).
...
etermsvar_call_to_success_builtin('is/2',(X is Y),Sv,Call,Proj,Succ):-
    (
        ...
        insert_type_name(NY,[],0),
        varset(Y,Svy),
        etermsvar_project(not_provided_Sg,Svy,not_provided_HvFv_u,Proj,Projy0),
        ...
        (
            ...
            varset(X,Svx),
            etermsvar_project(not_provided_Sg,Svx,not_provided_HvFv_u,Proj,Projx),
...
etermsvar_input_interface_(regtype(T),perfect,Acc,[T|Acc]):-
    functor(T,_,1),!,
    may_be_var(Acc).
etermsvar_input_interface_(regtype(T),perfect,Acc,[NonPT|Acc]):-
    functor(T,_,2),!,
    arg(1,T,V),
    ...
    arg(1,NonPT,V),
    may_be_var(Acc).
etermsvar_input_interface_(member(X,L),perfect,Acc,[P|Acc]):-
...
etermsvar_asub_to_native0([X:(_N,T)|ASub],[Type|OutputUser]):-
    ...
    etermsvar_asub_to_native0(ASub,OutputUser).
etermsvar_asub_to_native0([],[]).
...
etermsvar_collect_abstypes_abs([],Types,Types).
etermsvar_collect_abstypes_abs([_:(_,Type)|Abs],Types0,Types):-
    insert(Types0,Type,Types1),
    etermsvar_collect_abstypes_abs(Abs,Types1,Types).

etermsvar_rename_abstypes_abs([],_,[]).
etermsvar_rename_abstypes_abs([C|Call],Dict,[RenC|RenCall]):-
    Dict = (Types,_),
    C = Var:(_Name,Type),
...
etermsvar_identical_abstract(ASub1,ASub2):- ASub1==ASub2, !.
etermsvar_identical_abstract(ASub1,ASub2):-
    etermsvar_identical_abstract0(ASub1,ASub2).

etermsvar_identical_abstract0([X:(_Name0,Type0)|ASub0],[Y:(_Name1,Type1)|ASub1]):-
    X==Y,
    dz_equivalent_types(Type0,Type1),
    etermsvar_identical_abstract0(ASub0,ASub1).
etermsvar_identical_abstract0([],[]).
...
etermsvar_part_conc(A,ASub,NA,NASub):- 
    etermsvar_parc_conc_(A,ASub,NA,NASub,ASub).
 
etermsvar_parc_conc_(A,ASub,NA,NASub,NASubT):- 
    var(A),!,
...
etermsvar_parc_conc_(A,ASub,NA,NASub,NASubT):-  
    functor(A,F,Arity),
    functor(NA,F,Arity),
    etermsvar_parc_conc_arg(Arity,A,NA,ASub,NASub,NASubT).

etermsvar_parc_conc_arg(0,_A,_NA,_,ASub,ASub).
etermsvar_parc_conc_arg(Arity,A,NA,ASub,NASub,NASubT):-
    arg(Arity,A,Arg1),
    Arity1 is Arity - 1,
    etermsvar_parc_conc_(Arg1,ASub,NAArg,NASub,NASub2),     
    arg(Arity,NA,NAArg),    
    etermsvar_parc_conc_arg(Arity1,A,NA,ASub,NASub2,NASubT).
