...
:- doc(title, "Old Advanced Tutorial on Program Development and Optimization using the
    Ciao Preprocessor").

:- doc(author, "The Ciao Development Team").

% TODO: Review summary (think about what we want to say about cost)
:- doc(summary,
...
A @apl{ciaopp} session consists in the preprocessing of a file. The
session is governed by a menu, where you can choose the kind of
preprocessing you want to be done to your file among several analyses
and program transformations available. 
...
Except for the first and last lines, which refer to loading or saving a menu
configuration (a predetermined set of selected values for the different menu
options), each line corresponds to an option you can select, each having several
possible values. You can select either analysis (@tt{analyze}) or assertion
checking (@tt{check_assertions}) or certificate checking
(@tt{check_certificate}) or program optimization (@tt{optimize}), and you can
later combine the four kinds of preprocessing. The relevant options for the
@tt{action group} selected are then shown, together with the relevant flags.
...
PLAI abstract interpreter @cite{ai-jlp,effofai-toplas}, including extensions
...
The system includes several abstract analysis domains developed by
several groups in the LP and CLP communities and can infer information
on variable-level properties such as moded types, definiteness,
...
bounds on data structure sizes, as well as procedure-level properties
such as determinacy, termination, non-failure, and bounds on resource
...
@subsection{Module-aware Static Analysis Basics}

As mentioned before, @apl{CiaoPP} takes advantage of modular program structure
to perform more precise and efficient, incremental analysis. Consider the
program in Figure @ref{fig:qsort}, defining a module which exports the
@tt{qsort} predicate and imports predicates @tt{geq} and @tt{lt} from module
...
:- true pred append(A,B,C)
    ...
    => ( ground(A), mshare([[B,C]]) ).
@end{verbatim}
These @em{assertions} express, for example, that the
third and fourth arguments of @tt{partition} have \"output mode\":
when @tt{partition} is called (@tt{:}) they are free unaliased
variables and they are ground on success (@tt{=>}).  Also,
@tt{append} is used in a mode in which the first argument is input
(i.e., ground on call).  Also, upon success the arguments of
@tt{qsort} will share all variables (if any).

@subsection{Type Analysis}

@apl{CiaoPP} can infer (parametric) types for programs both at the predicate
level and at the literal level
@cite{gallagher-types-iclp94,set-based-absint-padl,eterms-sas02}.
...
As shown in the figures above, at the predicate level, assuming that we have
imported the @tt{lists} library, is:
...
:- true pred append(A,B,C)
     : ( list(A), list1(term,B), term(C) )
    => ( list(A), list1(term,B), list1(term,C) ).
@end{verbatim}
where @tt{term} is any term and prop @tt{list1} is defined
in @tt{library(lists)} as:
@begin{verbatim}
:- regtype list1(T,L) # \"@var{L} is a list of at least one @var{T}'s.\"
...
@end{verbatim}
We can use @tt{entry} assertions @cite{full-prolog-esop96}
to specify a restricted class of calls to the module entry points as
acceptable:
@begin{verbatim}
:- entry qsort(A,B) : (list(num, A), var(B)).
@end{verbatim}
This informs the analyzer that in all external calls to  @tt{qsort},
the first argument will be a list of numbers and the second a free
variable.  Note the use of builtin properties (i.e., defined in
modules which are loaded by default, such as  @tt{var},  @tt{num},
@tt{list}, etc.).  Note also that properties natively understood by
different analysis domains can be combined in the same assertion.
This assertion will aid goal-dependent analyses obtain more accurate
information. For example, it allows the type analysis to obtain the
following, more precise information:
@begin{verbatim}
:- true pred qsort(A,B)
     : ( list(num,A), term(B) )
...
:- true pred append(A,B,C)
     : ( list(num,A), list1(num,B), term(C) )
    => ( list(num,A), list1(num,B), list1(num,C) ).
@end{verbatim}


@subsection{Non-failure and Determinacy Analysis:}
@begin{alert}
@tt{'<'} not defined - something broken with trust of comp?
@end{alert}

@image{Figs/analysis-det-new}{671}{763}

@apl{CiaoPP} includes a non-failure analysis, based
on @cite{non-failure-iclp97} and @cite{nfplai-flops04}, which can
detect procedures and goals that can be guaranteed not to fail, i.e.,
to produce at least one solution or not terminate. It also can detect
predicates that are \"covered\", i.e., such that for any input
(included in the calling type of the predicate), there is at least one
clause whose \"test\" (head unification and body builtins) succeeds.
CiaoPP also includes a determinacy analysis based
on @cite{determ-lopstr04}, which can detect predicates which
produce at most one solution, or predicates whose clause tests are
mutually exclusive, 
even if they are not deterministic (because they call other predicates
that can produce more than one solution).
For example, the result of
these analyses for 
Figure @ref{fig:qsort} includes the following assertion:
@begin{verbatim}
:- true pred qsort(A,B)
     : ( list(num,A), var(B) ) => ( list(num,A), list(num,B) )
     + ( not_fails, covered, is_det, mut_exclusive ).
@end{verbatim}
(The @tt{+} field in @tt{pred} assertions can contain a conjunction
of global properties of the @em{computation} of the predicate.)
This result has been obtained using the @em{analyze} menu options
depicted in Figure @ref{fig:analysis-nfg-det}.

@image{Figs/analysis-cost}

@subsection{Size, Cost, and Termination Analysis}
...
proving @em{termination} of the predicate. 

As an example, the following assertion is part of the output of 
the upper bounds analysis:
@begin{verbatim}
:- true pred append(A,B,C)
     : ( list(num,A), list1(num,B), var(C) ) 
    => ( list(num,A), list1(num,B), list1(num,C), 
...
@tt{size_ub(C,length(B)+length(A))} means that an (upper) bound on the size of
the third argument of @tt{append/3} is the sum of the sizes of the first and
second arguments. The inferred upper bound on computational steps is the length
of the first argument of @tt{append/3}. The options that must be set in the
@em{analyze} menu of @apl{ciaopp} are shown in Figure @ref{fig:analysis-cost}.

The following is the
output of the lower-bounds analysis:
@begin{verbatim}
:- true pred append(A,B,C)
     : ( list(num,A), list1(num,B), var(C) ) 
    => ( list(num,A), list1(num,B), list1(num,C), 
...
The menu options are the same as in Figure @ref{fig:analysis-cost},
but selecting @tt{steps_lb} for the cost analysis option.
...
properties being inferred are in general undecidable at compile-time, the
inference technique used, abstract interpretation, is necessarily
@em{approximate}, i.e., possibly imprecise. On the other hand, such
approximations are also always guaranteed to be safe, in the sense that (modulo
bugs, of course) they are never @em{incorrect}: the properties stated in
inferred assertions do always hold of the program.

@section{Program Debugging and Assertion Validation}
@apl{ciaopp} is also capable of combined static and dynamic validation, and
...
Figure @ref{fig:chiprewhere} depicts the overall
architecture.
Hexagons represent the different tools involved and arrows indicate
the communication paths among them.

Program verification and detection of errors is first performed at
compile-time by 
inferring properties of the program via abstract
interpretation-based static analysis and comparing this information
against (partial) specifications 
written in terms of assertions (see @cite{ciaopp-sas03-journal-scp}
for a detailed description of the sufficient conditions used for
achieving this @apl{ciaopp} functionality).

Both the static and the dynamic checking are provably @em{safe} in
the sense that all errors flagged are definite violations of the
specifications.

@image{Figs/framework_all}
...
Assertions are a means of specifying @em{properties} which are (or should be)
true of a given predicate, predicate argument, and/or @em{program point}. If an
assertion has been proved to be true it has a prefix @tt{true}.
...
In CiaoPP properties are just predicates, which may be builtin or user defined.
...
to the module in which the property is used. In addition, properties which are
to be used in run-time checking (see later) should be defined by a (logic)
program or system builtin, and also visible. Properties declared and/or defined
in a module can be exported as any other predicate. For example:
@begin{verbatim}
:- prop list/1.
list([]).
list([_|L]) :- list(L).
...
defines the property \"list\". A list is an instance of a very useful class of
...
for debugging comes naturally after observing analysis outputs for
erroneous programs.  Consider the program in Figure~@ref{fig:qsort1}.
...
:- true pred qsort(A,B)
     : ( term(A), term(B) )
    => ( list(t113,A), list(^x,B) ).

:- regtype t113/1.
t113(A) :- arithexpression(A).
t113([]).
t113([A|B]) :- arithexpression(A), list(t113,B).
t113(e).
@end{verbatim}
where @tt{arithexpression} is a library property which describes arithmetic
expressions and @tt{list(^x,B)} means \"a list of @tt{x}'s.\" A new name
(@tt{t113}) is given to one of the inferred types, and its definition included,
because no definition of this type was found visible to the module. In any case,
the information inferred does not seem compatible with a correct definition of
@tt{qsort}, which clearly points to a bug in the program.
...
@apl{ciaopp} can find incompatibilities between the ways in which library
predicates are called and their intended mode of use, expressed in the form of
assertions in the libraries themselves. Also, the preprocessor can detect
inconsistencies in the program and check the assertions present in other modules
used by the program.

For example, we can turn on compile-time error checking and selecting
type and mode analysis for our tentative @tt{qsort} program in
Figure @ref{fig:qsort1}, by selecting the action
@em{check_assertions} as shown in
Figure @ref{fig:debugging-ctchecking1a}. 

By default, the option @em{Perform Compile-Time Checks} is set to
@em{ auto}, which means that the system will automatically detect the
analyses to be performed in order to check the program, depending on
the information available in the program assertions (in the example in
Figure @ref{fig:qsort1}, the entry assertion informs how the predicate
@tt{qsort/2} will be called using types and modes information only).
Using the default options,
and setting @em{Report Non-Verified Assrts} to @em{error}, we obtain
the following messages (and the system highlights the line which
produces the first of them, as shown:
@begin{verbatim}
...
Static compile-time checking of assertions in system libraries:
@image{Figs/debugging-ctchecking1a}

Results of compile-time checking of assertions in system libraries:
@image{Figs/debugging-ctchecking1b}

Results of compile-time checking after correcting first errors:
@image{Figs/debugging-ctchecking1c}

where the first message refers to the lines of the first clause of
@tt{qsort/2}, the second one to the second clause of @tt{partition/4}, and the last two messages correspond to the third
clause  @tt{partition/4}.

First and last messages warn that all calls to @tt{partition} and @tt{>=/2} will
fail, something normally not intended (e.g., in our case). The error message
indicates a wrong call to a builtin predicate, which is an obvious error. This
...
third arguments in reversed order -- the correct call is
...
details), producing as output the following assertions (refer also to 
Figure @ref{fig:chiprewhere}, output of the comparator): 
@begin{verbatim}
...
example.@footnote{Note that while property @tt{sorted_num_list}
...
Another very interesting feature of @apl{ciaopp} is the possibility of stating
assertions about the efficiency of the program which the system will try to
verify or falsify. This is done by stating lower and/or upper bounds on the
computational cost of predicates (given in number of execution steps). Consider
for example the naive reverse program in Figure @ref{fig:nrev}.

The naive reverse program:
@begin{verbatim}
...
Suppose that the programmer thinks that the cost of @tt{nrev} is
...
@tt{append} call. Since @tt{append} is linear, it causes
@tt{nrev} to be quadratic. We will show that @apl{ciaopp} can be used to
inform the programmer about this false idea about the cost of @tt{nrev}.
For example, suppose that the programmer adds the following \"check\"
assertion: 
@begin{verbatim}
:- check comp nrev(A,B) + steps_ub(length(A)+1).          
@end{verbatim}

In order to check cost assertions, we have to set specific analysis options to
@apl{ciaopp}. The way to do it is to set @tt{Customize Analysis Flags} to @tt{
on} when  @tt{Select Action Group} is set to @tt{check_assertions} in the menu
(see Figure @ref{fig:debugging-performance1}).

Access to analysis flags from the @em{check_assertions} menu:
@image{Figs/debugging-performance1}

The extended menu with the appropriate options is:

@image{Figs/debugging-performance2}
...
1.5 length(A) + 1} resolution steps (which is the cost analysis output), while
the assertion requires that it take at most @math{length(A)+1} resolution steps.
The cost function in the user-provided assertion is compared with the
lower-bound cost assertion inferred by analysis. This allows detecting the
inconsistency and proving that the program does not satisfy the efficiency
requirements imposed. Upper-bound cost assertions can also be proved to hold,
i.e., can be @em{checked}, by using upper-bound cost analysis rather than
lower-bound cost analysis. In such case, it holds when the upper-bound computed
by analysis is lower or equal than the upper-bound stated by the user in the
assertion. The converse holds for lower-bound cost assertions.

@apl{ciaopp} can also verify or falsify cost assertions expressing worst
...
Thanks to this functionality, @apl{ciaopp} can certify programs with
resource consumption assurances and also efficiently check such
certificates @cite{tutorial-europar04}.
...
provided they are @em{ multivariant} (the abstract interpreter that
@apl{ciaopp}uses, called PLAI @cite{ai-jlp,effofai-toplas}, has this
property, see Section @ref{Static Analysis and Program Assertions} for details).
...
@begin{alert}
Broken - @tt{auto_interface} error (required atom, found number)
@end{alert}
...
Consider the program of Figure @ref{fig:qsortnomod} (the
@tt{module} and @tt{entry} directives will be explained
later).
...
partition([E|R],C,[E|Left1],Right):- 
    E < C,  partition(R,C,Left1,Right).
partition([E|R],C,Left,[E|Roight1]):-
    E > C, partition(R,C,Left,Right1).
...
A possible parallelization 
obtained with the selected options in the menu depicted in
Figure @ref{fig:optimization-parallelization2} is:
...
@begin{alert}
Broken, hangs with @tt{ERROR: (infercost) : no cost analysis performed}
@end{alert}
